...
import { Plus, Edit3, Trash2, FolderKanban, Loader2, CheckCircle2, Clock, Circle } from 'lucide-react';
...
export function ProjectManager({ projects, onProjectsChange, userId, editProject, onEditComplete }: ProjectManagerProps) {
  const { language } = useUIStore();
  const [isDialogOpen, setIsDialogOpen] = useState(!!editProject);
  const [editingProject, setEditingProject] = useState<Project | undefined>(editProject);
  ...
  const texts = {
    ...
    projectNameNorwegian: language === 'no' ? 'Prosjektnavn (Norsk)' : 'Project Name (Norwegian)',
    description: language === 'no' ? 'Beskrivelse' : 'Description',
    descriptionNorwegian: language === 'no' ? 'Beskrivelse (Norsk)' : 'Description (Norwegian)',
    ...
    deleteConfirm: language === 'no' ? 'Er du sikker på at du vil slette dette prosjektet?' : 'Are you sure you want to delete this project?',
    deleteDescription: language === 'no' ? 'Denne handlingen kan ikke angres. Oppgaver i prosjektet blir ikke slettet.' : 'This action cannot be undone. Tasks in this project will not be deleted.',
    noProjects: language === 'no' ? 'Ingen prosjekter ennå' : 'No projects yet',
    createFirst: language === 'no' ? 'Opprett ditt første prosjekt' : 'Create your first project',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const projectData = {
        ...
        ...(formData.description_nb?.trim() && { description_nb: formData.description_nb.trim() }),
      };

      if (editingProject) {
        await projectsAPI.updateProject(editingProject.id.toString(), projectData);
      } else {
      ...
      onProjectsChange();
      setShowForm(false);
      setFormData({ name: '', name_nb: '', description: '', description_nb: '', status: 'todo' });
        ...
        <div className="space-y-4">
          ...
          {showForm && (
            <form onSubmit={handleSubmit} className="space-y-4 border-t pt-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="project-description-nb">{texts.descriptionNorwegian}</Label>
                <Textarea
