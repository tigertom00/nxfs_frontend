...
import { Job, UpdateJobPayload } from '@/lib/api';
import { MapPin, Loader2, Search, Trash2 } from 'lucide-react';
...
interface EditJobModalProps {
  ...
  onJobDeleted: (ordreNr: string) => void;
  job: Job | null;
}
...
export function EditJobModal({
  ...
  useEffect(() => {
    if (isOpen && job) {
      setFormData({
        tittel: job.tittel || '',
        adresse: job.adresse || '',
        postnummer: job.postnummer || '',
        poststed: job.poststed || '',
        telefon_nr: job.telefon_nr || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const updatedJob = await jobsAPI.updateJob(job.ordre_nr, formData);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await jobsAPI.deleteJob(job.ordre_nr);
      onJobDeleted(job.ordre_nr);
  ...
  const handleInputChange = (field: keyof UpdateJobPayload, value: any) => {
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-md mx-auto max-h-[90vh] flex flex-col">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>Edit Job #{job.ordre_nr}</DialogTitle>
          </DialogHeader>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Job #{job.ordre_nr}?</AlertDialogTitle>
            <AlertDialogDescription>
