...
export function useEnvironmentVariables() {
  ...
  const loadVariables = () => {
    try {
      ...
      if (saved) {
        ...
        const uniqueVariables = migratedVariables.filter((v: EnvironmentVariable, index: number, self: EnvironmentVariable[]) =>
          index === self.findIndex((t: EnvironmentVariable) => t.key === v.key)
        )

        setVariables(uniqueVariables)
        localStorage.setItem("environmentVariables", JSON.stringify(uniqueVariables))
      } else {
  ...
  const saveVariables = (newVariables: EnvironmentVariable[]) => {
    try {
      ...
      localStorage.setItem("environmentVariables", JSON.stringify(newVariables))
    } catch (error) {
