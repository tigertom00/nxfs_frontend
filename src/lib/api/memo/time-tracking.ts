...
export const timeTrackingAPI = {
  ...
  getTimeEntries: async (params?: TimeEntrySearchParams): Promise<GetTimeEntriesResponse> => {
    try {
  ...
  // Create time entry
  createTimeEntry: async (entryData: CreateTimeEntryPayload): Promise<CreateTimeEntryResponse> => {
    try {
  ...
  updateTimeEntry: async (
    ...
    try {
      const response = await api.patch(`/app/memo/timeliste/${entryId}/`, entryData);
