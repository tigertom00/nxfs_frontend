...
import { Send, Copy, Download, History, Save, Trash2, Plus, X, Settings, RotateCcw, Terminal, Play } from "lucide-react"
...
export default function ApiTesterPage() {
  ...
  const [responseFormat, setResponseFormat] = useState<"json" | "raw" | "preview">("json")
  const { savedRequests, saveRequest, deleteRequest } = useSavedRequests()
  const { variables, substituteVariables, saveVariables } = useEnvironmentVariables()
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      toast.success(`Request sent successfully - Status: ${response.status} ${response.statusText}`)
    } catch (error) {
      const responseTime = Date.now() - startTime
  ...
  const formatResponse = (responseText: string, format: "json" | "raw" | "preview") => {
    switch (format) {
      case "json":
  ...
  const parseAndFillForm = () => {
    ...
    if (authHeader) {
      const tokenMatch = authHeader.match(/Bearer\s+(.+)/i) || authHeader.match(/Token\s+(.+)/i)
      if (tokenMatch) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Professional API testing tool</p>
            </div>
          </div>
        </div>
            ...
            <Card className="hover-lift border-l-4 border-l-api-accent-border">
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={sendRequest} disabled={isRequesting} className="api-gradient hover:opacity-90">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button
                            variant={responseFormat === "raw" ? "default" : "outline"}
                            size="sm"
                          ...
                          <Button
                            variant={responseFormat === "preview" ? "default" : "outline"}
                            size="sm"
          ...
          <div className="space-y-6">
            ...
            <Card className="hover-lift border-l-4 border-l-api-accent-border">
              ...
              <CardContent className="space-y-2">
                ...
                <Button variant="outline" className="w-full justify-start" onClick={exportLogs}>
                ...
                >
                  <Button variant="outline" className="w-full justify-start">
                    ...
                    Environment Variables ({variables.filter(v => v.enabled).length})
                  </Button>
                </EnvironmentVariablesDialog>
              </CardContent>
            </Card>
            ...
            <Card className="hover-lift border-l-4 border-l-api-accent-border">
              <CardHeader>
                ...
                <CardDescription>Enter a curl command to fill the form or send directly</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                />
                <div className="flex gap-2">
                  <Button variant="outline" onClick={parseAndFillForm} className="flex-1">
                    <Terminal className="w-4 h-4 mr-2" />
                    Fill Form
                  </Button>
                  <Button onClick={parseAndSendCurl} className="flex-1 api-gradient hover:opacity-90">
