...
export default function LLMProvidersPage() {
  ...
  const texts = {
    ...
    subtitle: language === 'no' ? 'En omfattende liste over AI-modeller og plattformer' : 'A comprehensive list of AI models and platforms',
    search: language === 'no' ? 'Søk etter leverandører...' : 'Search providers...',
  ...
  const providers: LLMProvider[] = [
    {
      ...
      strengths: [
        language === 'no' ? 'Utmerkede skriveferdigheter' : 'Excellent writing capabilities',
        language === 'no' ? 'Sterk i koding og problemløsning' : 'Strong in coding and problem solving',
        language === 'no' ? 'Bilde- og dokumentanalyse' : 'Image and document analysis',
        language === 'no' ? 'Stort økosystem med plugins' : 'Large ecosystem with plugins'
      ],
    ...
    },
    {
      ...
      strengths: [
        ...
        language === 'no' ? 'Sterk i dokumentanalyse' : 'Excellent at document analysis',
        language === 'no' ? 'Fokus på sikkerhet og etikk' : 'Focus on safety and ethics',
        language === 'no' ? 'God til komplekse oppgaver' : 'Good at complex tasks'
      ],
      category: texts.general,
      pricing: texts.freemium,
    ...
    },
    {
      ...
      strengths: [
        language === 'no' ? 'Multimodal (tekst, bilder, lyd)' : 'Multimodal (text, images, audio)',
    ...
    },
    {
      ...
      strengths: [
        language === 'no' ? 'Dyp Office-integrasjon' : 'Deep Office integration',
    ...
    },
    {
      ...
      strengths: [
        ...
        language === 'no' ? 'Rask informasjonstilgang' : 'Quick information access'
      ],
      category: texts.research,
      pricing: texts.freemium,
    ...
    },
    {
      ...
      strengths: [
        language === 'no' ? 'Mange modeller tilgjengelige' : 'Many models available',
  ...
  const categories = [texts.allCategories, texts.general, texts.coding, texts.creative, texts.research, texts.multimodal, texts.openSource, texts.enterprise];

  const filteredProviders = providers.filter(provider => {
    const matchesSearch = provider.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         provider.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         provider.strengths.some(strength => strength.toLowerCase().includes(searchTerm.toLowerCase()));
    
    const matchesCategory = selectedCategory === texts.allCategories || provider.category === selectedCategory;
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case texts.coding: return <Code className="h-4 w-4" />;
      case texts.creative: return <Palette className="h-4 w-4" />;
      case texts.research: return <Book className="h-4 w-4" />;
      case texts.multimodal: return <Image className="h-4 w-4" />;
      case texts.openSource: return <Rocket className="h-4 w-4" />;
      case texts.enterprise: return <Shield className="h-4 w-4" />;
  ...
  const getPricingColor = (pricing: string) => {
    switch (pricing) {
      case texts.free: return 'bg-green-100 text-green-800';
      case texts.freemium: return 'bg-blue-100 text-blue-800';
      case texts.paid: return 'bg-purple-100 text-purple-800';
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {categories.map((category) => (
                ...
                >
                  {category !== texts.allCategories && getCategoryIcon(category)}
                  <span>{category}</span>
                </Button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProviders.map((provider) => (
              <Card key={provider.id} className="h-full hover:shadow-lg transition-shadow">
                <CardHeader>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2 mt-1">
                          ...
                          <Badge className={`text-xs ${getPricingColor(provider.pricing)}`}>
                            {provider.pricing}
                          </Badge>
                        </div>
                      </div>
          ...
          {filteredProviders.length === 0 && (
            <div className="text-center py-12">
              <h3 className="text-xl font-semibold mb-2">
                {language === 'no' ? 'Ingen leverandører funnet' : 'No providers found'}
              </h3>
